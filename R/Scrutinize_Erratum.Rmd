...
The model controls for multiple data points per species (random intercept), but not per study site. The model residuals are confounded by site
...
The model with a random effect that has as many levels (`r length(unique(paste(data$species, data$pop_nr)))`)
 as the rows in the dataset (`r nrow(data)`). Despite the claims in the Erratum, the residuals from this model are still confounded by study site.
```{r erratum,  warning = FALSE, message = FALSE}
idx <- match(data$species, rownames(phyloMat)) 
matchedPhyloMat <- phyloMat[idx,idx]
data2 <- data
data2$speciesM <- paste(data$species, data$pop_nr) #length(unique(data2$speciesM))
