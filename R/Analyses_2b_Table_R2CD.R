...
# al's data to generates Table R1CD within part
...
# DPR - 2 way - site + species control
  md2= lmer(log(DPR) ~ log(N_nests) + scale(mean_year)*Belt + (1|site)+(1|species),  data = d) 
  summary(glht(md2))

# TPR - 2 way - site + species control
  mt2= lmer(TPR ~ log(N_nests) + scale(mean_year)*Belt + (1|site)+(1|species),  data = d) 
  summary(glht(mt2))

# DPR - 2 way - no site + species control
  md3= lm(log(DPR) ~ log( N_nests) + scale(mean_year)*Belt ,  data = d) 
  summary(glht(md3))

# TPR - 2 way - no site + species control
  mt3= lm(TPR ~ log( N_nests) + scale(mean_year)*Belt ,  data = d) 
  summary(glht(mt3))

# export results
  o1 = m_out(name = "dpr lmm", model = md2, round_ = 3, nsim = 5000, aic = FALSE)[1:6]
  o2 = m_out(name = "tpr lmm", model = mt2, round_ = 3, nsim = 5000, aic = FALSE)[1:6]
  o3 = m_out_lm(name = "dpr lm", model = md3, round_ = 3, nsim = 5000, aic = FALSE)[1:6]
  o4 = m_out_lm(name = "tpr lm", model = mt3, round_ = 3, nsim = 5000, aic = FALSE)[1:6]

  o1$p_value = NA
  o1$p_value[1:11] = round(summary(glht(md2))$test$pvalues,2)
  o2$p_value = NA
  o2$p_value[1:11] = round(summary(glht(mt2))$test$pvalues,2)
  o3$p_value = NA
  o3$p_value[1:11] = round(summary(glht(md3))$test$pvalues,2)
  o4$p_value = NA
  o4$p_value[1:11] = round(summary(glht(mt3))$test$pvalues,2)

  write_xlsx(rbind(o1,o2,o3,o4), 'Outputs/Table_R2CD.xlsx')
